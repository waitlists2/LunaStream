...
const HybridTVHeader: React.FC<HybridTVHeaderProps> = ({
  ...
  // Fetch season details when a season is selected (>0)
  useEffect(() => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-purple-900 to-pink-900">
      ...
      <div className="relative z-10 p-6 md:p-8">
        {/* Poster */}
        <div className="flex-shrink-0 w-48 md:w-64 mb-4 md:mb-0">
          <div className="relative group">
            <img
              src={tmdb.getImageUrl(displayData.poster, "w500") || "/placeholder.svg"}
              alt={displayData.title}
              className="w-full h-auto object-cover rounded-xl shadow-2xl transition-transform group-hover:scale-105"
            />
        ...
        <div className="flex-1">
          ...
          {/* Season Selector */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-300 mb-2">{t.select_season}</label>
            <div className="relative w-full md:w-64 season-dropdown">
              <button
                onClick={() => setDropdownOpen(prev => !prev)}
                className="w-full bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg px-4 py-2 text-white flex justify-between items-center"
              >
                <span>
                  {selectedSeason === 0
                    ? "Show Overview"
                ...
                <ul className="absolute z-20 mt-2 w-full bg-gray-900/90 border border-white/20 backdrop-blur-sm rounded-lg shadow-xl max-h-64 overflow-y-auto">
                  ...
                  {availableSeasons.map(s => (
                    <li
                      ...
                      className={`px-4 py-2 text-white hover:bg-pink-500/20 cursor-pointer ${selectedSeason === s.season_number ? "bg-white/10" : ""}`}
                    >
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-300">{displayData.overview}</p>
          </div>

          {/* Genres */}
          <div className="flex flex-wrap gap-2">
            {displayData.genres.map(genre => (
              <span key={genre.id} className="bg-white/20 backdrop-blur-sm text-white px-3 py-1 rounded-full text-sm">{genre.name}</span>
            ))}
          </div>
        </div>
      </div>
